...
const validateSNoExists = (rule: any, value: any, callback: any) => {
  ...
  Api.tuwenzhibotestxpath.getAll({telA: value}).then((res) => {
    if (res.data.count > 0) {
      console.log(value)
...
const getUser = () => {
  ...
  // axios请求
  Api.tuwenzhibotestxpath.getAll(params)
      .then((res) => {
        // 开启loading
        openLoading()
        // 判断是否成功
...
const laterCommit = () => {
  ...
  proxy.$refs.dataFormRef.validate((valid: boolean)=>{
    ...
    if (Data.isEdit) {
      ...
      openLoading()
      // 修改
      Api.tuwenzhibotestxpath.edit(Data.dataForm.id, Data.dataForm).then((res) => {
        if (res.status === 201) {
          // 关闭loading
          closeLoading()
          // 重新加载数据
      ...
      // 添加
      Api.tuwenzhibotestxpath.add(Data.dataForm).then((res) => {
        // 关闭loading
        closeLoading()
        // 重新加载数据
        getUser();
  ...
  >
    ...
    <el-table-column label="操作" align="center" width="150px">
      <template #default="scope">
        ...
        <!--        <el-button type="danger" :icon="Delete" @click="delTuwenzhibotestxpath(scope.row)" circle size="small"/>-->
      </template>
