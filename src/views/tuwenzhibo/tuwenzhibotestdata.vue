...
const validateSNoExists = (rule: any, value: any, callback: any) => {
  ...
  Api.tuwenzhibotestdata.getAll({telA: value}).then((res) => {
    if (res.data.count > 0) {
      console.log(value)
...
const getUser = () => {
  ...
  // axios请求
  Api.tuwenzhibotestdata.getAll(params)
      .then((res) => {
        // 开启loading
        openLoading()
        // 判断是否成功
...
const laterCommit = () => {
  ...
  proxy.$refs.dataFormRef.validate((valid: boolean) => {
    ...
    if (Data.isEdit) {
      ...
      openLoading()
      // 修改
      Api.tuwenzhibotestdata.edit(Data.dataForm.id, Data.dataForm).then((res) => {
        if (res.status === 201) {
          // 关闭loading
          closeLoading()
          // 重新加载数据
      ...
      // 添加
      Api.tuwenzhibotestdata.add(Data.dataForm).then((res) => {
        if (res.status === 201) {
          // 关闭loading
          closeLoading()
          // 重新加载数据
  ...
  >
    ...
    <el-table-column label="操作" align="center" width="150">
      <template #default="scope">
        ...
        <el-button type="danger" :icon="Delete" @click="delTuwenzhibotestdata(scope.row)" circle size="small"/>
      </template>
