...
const getUser = () => {
  ...
  // axios请求
  Api.loupankuhoutaiofficialxpath.getAll(params)
      .then((res) => {
        // 判断是否成功
        console.log('成功', res)
        if (res.status === 200) {
...
const laterCommit = () => {
  ...
  proxy.$refs.dataFormRef.validate((valid: boolean)=>{
    ...
    if (Data.isEdit) {
      // 修改
      Api.loupankuhoutaiofficialxpath.edit(Data.dataForm.id, Data.dataForm).then((res) => {
        if (res.status === 201) {
          // 重新加载数据
          getUser();
          // 关闭弹出层
      ...
      // 添加
      Api.loupankuhoutaiofficialxpath.add(Data.dataForm).then((res) => {
        // 重新加载数据
        getUser();
        // 关闭弹出层
        closeLater();
  ...
  >
    ...
    <el-table-column label="操作" align="center" width="150px">
      <template #default="scope">
        ...
        <!--        <el-button type="danger" :icon="Delete" @click="delLoupankuhoutaiofficialxpath(scope.row)" circle size="small"/>-->
      </template>
